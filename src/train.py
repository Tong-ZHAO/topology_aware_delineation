...
def val_epoch(net, dataloader, vgg_loss, pred_loss, mu, vis, epoch, cuda):
    ...
    print("Epoch %d (Val): total loss - %f, pred loss - %f, vgg loss - %f, bce_loss - %f" % 
          (epoch, all_val_loss / N, all_pred_loss / N, all_vgg_loss / N, all_bce_loss / N))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--layers', nargs='+', default = [4, 9, 18, 27], type = int, help = 'the extracted features from vgg')
    ...
    if opt.model != 'None':
        model.unet.load_state_dict(torch.load(opt.model)["net"])
